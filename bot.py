import telebot
import datetime
import time
import threading
import random

bot = telebot.TeleBot("7928915156:AAGZhxCCY4T2k7p1d6ypkHiMxbuWBaoRukc")

# Словарь для хранения данных о воде, выпитой пользователем
user_water_intake = {}


@bot.message_handler(commands=["start"])
def start_message(message):
    bot.reply_to(message, "Привет! Я чат бот, который будет напоминать тебе пить водичку!")
    reminder_thread = threading.Thread(target=send_reminders, args=(message.chat.id,))
    reminder_thread.start()


@bot.message_handler(commands=["help"])
def help_message(message):
    help_text = (
        "/start - Начать общение с ботом\n"
        "/fact - Получить случайный факт о воде\n"
        "/total - Узнать, сколько воды вы выпили за день\n"
        "Просто напишите количество выпитой воды, чтобы сохранить данные (например, '250 мл')."
    )
    bot.reply_to(message, help_text)


@bot.message_handler(commands=["fact"])
def fact_message(message):
    facts = [
        "**Вода на Земле может быть старше самой Солнечной системы**: Исследования показывают, что от 30% до 50% воды в наших океанах возможно присутствовала в межзвездном пространстве еще до формирования Солнечной системы около 4,6 миллиарда лет назад.",
        "**Горячая вода замерзает быстрее холодной**: Это явление известно как эффект Мпемба. Под определенными условиями горячая вода может замерзать быстрее, чем холодная, хотя ученые до сих пор полностью не разгадали механизм этого процесса.",
        "**Больше воды в атмосфере, чем во всех реках мира**: Объем водяного пара в атмосфере Земли в любой момент времени превышает объем воды во всех реках мира вместе взятых. Это подчеркивает важную роль атмосферы в гидрологическом цикле, перераспределяя воду по планете."
    ]
    random_fact = random.choice(facts)
    bot.reply_to(message, f"Лови факт о воде: {random_fact}")


@bot.message_handler(commands = ["total"])

def total_water(message):
    user_id = message.chat.id
    total = user_water_intake.get(user_id, 0)
    bot.reply_to(message, f"Вы выпили {total} мл воды за сегодня.")


@bot.message_handler(func=lambda message: True)
def track_water_intake(message):
    user_id = message.chat.id
    try:
        # Извлекаем количество воды из сообщения
        amount = int(message.text.split()[0])  # Предполагаем, что сообщение начинается с числа
        if amount <= 0:
            raise ValueError("Количество должно быть положительным.")

        # Увеличиваем общее количество выпитой воды
        if user_id in user_water_intake:
            user_water_intake[user_id] += amount
        else:
            user_water_intake[user_id] = amount

        bot.reply_to(message, f"Вы добавили {amount} мл воды. Всего: {user_water_intake[user_id]} мл.")

    except (ValueError, IndexError):
        bot.reply_to(message, "Пожалуйста, введите количество воды в миллилитрах (например, '250').")


def send_reminders(chat_id):
    first_rem = "09:00"
    second_rem = "14:00"
    end_rem = "18:00"
    while True:
        now = datetime.datetime.now().strftime("%H:%M")
        if now == first_rem or now == second_rem or now == end_rem:
            bot.send_message(chat_id, "Напоминание - выпей стакан воды")
            time.sleep(61)
        time.sleep(1)


bot.polling(none_stop=True)
